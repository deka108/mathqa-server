"""
# Name:           check_answer_api/utilities/trigonometry_prove_checker.py
# Description:
...
def check(correct_answer, user_answer, important_step=None):
    ...
    for step_answer in user_answer:
        ...
        if not correct:
            return False, step
        step += 1
    # Last step should be equal to the RHS of the question
    last_step = user_answer[len(user_answer) - 1]
    last_step_standard = answer_transformer.transform_latex_to_sympy(
        last_step, mode="Mathquill only")
    correct_answer_standard = answer_transformer.transform_latex_to_sympy(
        correct_answer, mode="Mathquill only")
    if last_step_standard != correct_answer_standard:
        return False, len(user_answer) - 1
