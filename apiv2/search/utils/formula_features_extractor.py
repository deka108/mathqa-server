...
def _write_dom_to_tempfile(root):
    """
...
def generate_features(latex_str, raw=False):
    ...
    """
    try:
        mathml_str = _generate_mathmlstr(latex_str)
        dom_tree = _construct_domtree(mathml_str)

        # Extract features from the formula
        sem_features, struc_features, const_features, var_features = \
            _extract_features(dom_tree)
...
def _generate_mathmlstr(latex_str):
    """
...
def _construct_domtree(mathml_str):
    """
...
def _extract_features(dom_tree):
    """
    ...
    tf = _write_dom_to_tempfile(dom_tree)
    ...
    for event, element in context:
        ...
        if event == 'start' and element.tag == 'mo':
            ...
            if len(stack_node) > 3:
                struc_features += _extract_structural_features(stack_node,
                                                               element)
            ...
            if element.tag == 'mn':
                cn_features.add(
                    _extract_structural_features(stack_node, element,
                                                 True) + 'cn')
            elif element.tag == 'mi':
                var_features.add(
                    _extract_structural_features(stack_node, element, True)
                    + 'var')
    tf.close()
...
def _extract_structural_features(stack_node, element, cn_var=False):
    """
